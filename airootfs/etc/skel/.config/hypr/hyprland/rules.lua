...
local function get_screen_dimensions()
    if hl and hl.get_active_monitor then
        local ok, mon = pcall(hl.get_active_monitor)
        if ok and mon and mon.width and mon.height then
    ...
    if hl and hl.get_monitors then
        ...
        if ok and type(mons) == "table" and #mons > 0 then
            local mon = mons[1]
            local scale = tonumber(mon.scale) or 1.0
            return math.floor(mon.width / scale), math.floor(mon.height / scale)
        end
...
local function parse_dimension(val, total)
...
local function resolve_gaps(gaps)
    ...
    local screen_w, screen_h = get_screen_dimensions()

    local top = gaps.top or gaps.y or gaps.vertical or 0
    local bottom = gaps.bottom or gaps.y or gaps.vertical or 0
    local left = gaps.left or gaps.x or gaps.horizontal or 0
    local right = gaps.right or gaps.x or gaps.horizontal or 0

    local top_px = parse_dimension(top, screen_h)
    local bottom_px = parse_dimension(bottom, screen_h)
    local left_px = parse_dimension(left, screen_w)
    local right_px = parse_dimension(right, screen_w)

    -- Safety check: ensure the window is never squeezed on small screens
    local max_side_gap = math.floor((screen_w - math.min(500, screen_w * 0.7)) / 2)
    if max_side_gap > 0 then
        left_px = math.min(left_px, max_side_gap)
        right_px = math.min(right_px, max_side_gap)
    end
...
local singleWindowGaps = resolve_gaps(vars.singleWindowGapsOut)

hl.workspace_rule({ workspace = "w[tv1]s[false]", gaps_out = singleWindowGaps })
hl.workspace_rule({ workspace = "f[1]s[false]", gaps_out = singleWindowGaps })
