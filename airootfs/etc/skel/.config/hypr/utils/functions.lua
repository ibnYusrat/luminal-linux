...
local function adjust_gaps(delta)
    return function()
        local gaps_in_cfg = hl.get_config("general:gaps_in") or {}
        local gaps_out_cfg = hl.get_config("general:gaps_out") or {}
        local current_in = gaps_in_cfg.top or 5
        local current_out = gaps_out_cfg.top or 10
        local new_in = math.max(0, current_in + delta)
        local new_out = math.max(0, current_out + delta * 2)
        hl.config({
            general = {
                gaps_in  = new_in,
                gaps_out = new_out,
