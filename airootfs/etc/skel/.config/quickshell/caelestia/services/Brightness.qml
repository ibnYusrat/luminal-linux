...
Singleton {
    ...
    component Monitor: QtObject {
        ...
        readonly property Timer pollTimer: Timer {
            ...
            repeat: true
            running: true
            onTriggered: monitor.initBrightness(true)
        }
        ...
        function initBrightness(force: bool): void {
            const now = Date.now();
            if (!force && lastInitTime > 0 && (now - lastInitTime) < 10000)
                return;
            lastInitTime = now;
        ...
        onBusNumChanged: initBrightness(false)
        Component.onCompleted: initBrightness(true)
    }
